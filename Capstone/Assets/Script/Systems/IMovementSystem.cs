...
namespace SkateGame
{
    ...
    public class MovementSystem : AbstractSystem, IMovementSystem
    {
        ...
        private void OnMoveInput(MoveInputEvent evt)
        {
            ...
            string currentState = playerController.stateMachine.GetMovementStateName();
        ...
        public void ApplyGroundMovement(float horizontalInput)
        {
            ...
            float targetSpeed = horizontalInput * playerModel.Config.Value.moveSpeed;
            ...
            if (Mathf.Abs(horizontalInput) > 0.01f)
            {
                ...
                {
                    // (2) 同方向，加速逼近目标速度
                    newSpeed = Mathf.MoveTowards(currentSpeed, targetSpeed, playerModel.Config.Value.groundAccel * Time.fixedDeltaTime);
                }
            }
            // ------------------------
            // 2. 没有输入时 → 滑行一段再停下
            ...
            {
                float groundDecel = playerModel.Config.Value != null ? playerModel.Config.Value.groundDecel : 10f;
                newSpeed = Mathf.MoveTowards(currentSpeed, 0, groundDecel * Time.fixedDeltaTime);
            }
        ...
        public void ApplyAirMovement(float horizontalInput)
        {
            ...
            // 空中移动：施加力，并限制最大速度
            if (Mathf.Abs(horizontalInput) > 0.01f)
            {rb.AddForce(Vector2.right * horizontalInput * playerModel.Config.Value.airControlForce, ForceMode2D.Force);
                
                // 控制最大水平速度
                float maxSpeed = playerModel.Config.Value.maxAirHorizontalSpeed;
